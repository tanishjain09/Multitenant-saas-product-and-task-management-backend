...
class UserServiceTest {
    ...
    @Test
    void createInitialUser_Success() {
        // Arrange
        String tenantKey = "test-tenant";
        when(tenantRepository.findByTenantKey(tenantKey)).thenReturn(Optional.of(testTenant));
        when(userRepository.existsByTenantAndEmail(testTenant, validRequest.getEmail())).thenReturn(false);
        when(userRepository.existsByTenantAndUsername(testTenant, validRequest.getUsername())).thenReturn(false);
        when(passwordEncoder.encode(anyString())).thenReturn("encodedPassword");
        ...
        // Act
        User result = userService.createInitialUser(tenantKey, validRequest);
    ...
    @Test
    void createInitialUser_TenantNotFound_ThrowsException() {
        // Arrange
        String tenantKey = "nonexistent-tenant";
        when(tenantRepository.findByTenantKey(tenantKey)).thenReturn(Optional.empty());

        // Act & Assert
        ResponseStatusException exception = assertThrows(
                ResponseStatusException.class,
                () -> userService.createInitialUser(tenantKey, validRequest)
        );
        assertEquals(HttpStatus.BAD_REQUEST, exception.getStatusCode());
        assertTrue(exception.getReason().contains("Tenant not found"));
    }
    ...
    void getAllUsers_Success() {
        ...
        when(userRepository.findAllByTenant_Id(testTenantId)).thenReturn(Arrays.asList(user1, user2));
        ...
        verify(userRepository, times(1)).findAllByTenant_Id(testTenantId);
    }

    @Test
    void getAllUsers_TenantContextMissing_ThrowsException() {
        // Arrange
        TenantContext.clear();
        ...
        assertEquals(HttpStatus.UNAUTHORIZED, exception.getStatusCode());
        assertTrue(exception.getReason().contains("Tenant context not set"));
    }
}